...
public class KhmerSegmentationService extends
	...
	public KhmerSegmentationService() throws Exception {
	  ...
	  // khmerworddetector = new MMAModel(Integer.valueOf(KhDictionary.MIX_DICTIONARY).intValue()); //faster, less accurate
	...
	private Collection<Morpheme> analyze(String text) throws Exception
	{
		...
		java.util.Vector segments = khmerworddetector.getBestSegmentation
      		  (text, KhDictionary.MIX_DICTIONARY, SEG_METHOD.WITH_EDIT_DISTANCE);
	...
	private  static boolean getAlgorithm(String filename)
	    {
	      ...
	      try
	      {
	        if (checkHeader(filename))
	        {
	          ...
	          buffer = "";
	          while (((buffer = reader.readLine()) != null) || (algo.equals(""))) {
	            if (!buffer.equals("")) {
	...
	private static boolean checkHeader(String fileName)
	    	    throws Exception
	    	  {
	    	    String buffer = "";
	    	    try
	    	    {
	    	      ...
	    	      do
	    	      {
	    	        ...
	    	        if ((!buffer.equals(null)) && 
	    	          (!buffer.equals("")) && 
	    	          (buffer.trim().equalsIgnoreCase("# Author : PAN Localization Cambodia"))) {
	    	          v = 1;
	    	        }
	    	      ...
	    	      if (!buffer.equals(null))
	    	      {
	    	        if (!buffer.equals(""))
	    	        {
	    	          if (buffer.trim().equalsIgnoreCase("# Author : PAN Localization Cambodia"))
	    	          {
	    	            reader.close();
	    	            return true;
	    	          }
